...
def training_loop(train_dataloader, opts):
    ...
    for epoch in range(opts.num_epochs):
        ...
        for batch in train_dataloader:
            ...
            D_real_loss = (D(real_images)-1).pow(2).mean()

            # 2. Sample noise
            noise = sample_noise(opts.noise_size)       # [B, noise_size, 1, 1]

            # 3. Generate fake images from the noise
            fake_images = G(noise)

            # 4. Compute the discriminator loss on the fake images
            D_fake_loss = D(fake_images).pow(2).mean()
            ...
            G_loss = (D(fake_images)-1).pow(2).mean() * 2
